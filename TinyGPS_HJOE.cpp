...
HABJOE VERION OF THE TINYGPS LIBRARY - added init code to set required config
...
// Public Methods //////////////////////////////////////////////////////////////
void TinyGPS_HJOE::init(void)
{
  ck_a=0;
	ck_b=0;

	//Airborne Mode    = 0x06			
	settingsArray[0] = 0x06;
	//3.3Hz     = 				== 3
	settingsArray[1] = 0x2C;
	settingsArray[2] = 0x01;
	//Baud 115200 = 00 C2 01					== 6	
	//settingsArray[3] = 0x00;
	//settingsArray[4] = 0xC2;
	//settingsArray[5] = 0x01	;
	//NMEA GLL Messages = 0x00						off
	settingsArray[6] = 0x00;	
	//NMEA GSA Messages = 0x00						off
	settingsArray[7] = 0x00	;
	//NMEA GSV Messages  = 0x00						off
	settingsArray[8] = 0x00	;
	//NMEA RMC Messages = 0x00						on
	settingsArray[9] = 0x10;
	//NMEA VTG Messages = 0x00						off
	settingsArray[10] = 0x00;		
...
void TinyGPS_HJOE::configureUbloxSettings() {
  ...
  //byte setPortRate[] = {0xB5, 0x62, 0x06, 0x00, 0x14, 0x00, 0x01, 0x00, 0x00, 0x00, 0xD0, 0x08, 0x00, 0x00, settingsArrayPointer[3], settingsArrayPointer[4], settingsArrayPointer[5], 0x00, 0x07, 0x00, 0x03, 0x00, 0x00, 0x00, 0x00, 0x00, 0xC0, 0x7E};
  //calcChecksum(&setPortRate[2], sizeof(setPortRate) - 4);
  //0000  B5 62 06 00 14 00 01 00 00 00 D0 08 00 00 00 C2 01 00 07 00 03 00 00 00 00 00 C0 7E   
...
bool TinyGPS_HJOE::encode(char c)
{
...
#ifndef _GPS_NO_STATS
void TinyGPS_HJOE::stats(unsigned long *chars, unsigned short *sentences, unsigned short *failed_cs)
{
...
// internal utilities
//
int TinyGPS_HJOE::from_hex(char a) 
{
  if (a >= 'A' && a <= 'F')
    return a - 'A' + 10;
  else if (a >= 'a' && a <= 'f')
...
unsigned long TinyGPS_HJOE::parse_decimal()
{
  char *p = _term;
  bool isneg = *p == '-';
  if (isneg) ++p;
...
unsigned long TinyGPS_HJOE::parse_degrees()
{
  char *p;
...
bool TinyGPS_HJOE::term_complete()
{
  if (_is_checksum_term)
  {
...
long TinyGPS_HJOE::gpsatol(const char *str)
{
  long ret = 0;
...
int TinyGPS_HJOE::gpsstrcmp(const char *str1, const char *str2)
{
...
/* static */
float TinyGPS_HJOE::distance_between (float lat1, float long1, float lat2, float long2) 
{
...
float TinyGPS_HJOE::course_to (float lat1, float long1, float lat2, float long2) 
{
...
const char *TinyGPS_HJOE::cardinal (float course)
{
...
void TinyGPS_HJOE::get_position(long *latitude, long *longitude, unsigned long *fix_age)
{
...
void TinyGPS_HJOE::get_datetime(unsigned long *date, unsigned long *time, unsigned long *age)
{
  if (date) *date = _date;
  if (time) *time = _time;
...
void TinyGPS_HJOE::f_get_position(float *latitude, float *longitude, unsigned long *fix_age)
{
  long lat, lon;
...
void TinyGPS_HJOE::crack_datetime(int *year, byte *month, byte *day, 
...
void TinyGPS_HJOE::sendUBX(uint8_t *MSG, uint8_t len) {
  for(int i=0; i<len; i++) {
		Serial1.write(MSG[i]);
...
//byte TinyGPS_HJOE::getUBX_ACK(byte *msgID) {
uint8_t TinyGPS_HJOE::getUBX_ACK(uint8_t *MSG) {
  byte CK_A = 0, CK_B = 0;
  byte incoming_char;
  ...
  if (MSG[0] == ackPacket[6] && MSG[1] == ackPacket[7] && CK_A == ackPacket[8] && CK_B == ackPacket[9]) {
...
long TinyGPS_HJOE::join_4_bytes(unsigned char Buffer[]) {
  union long_union {
	int32_t dword;
	uint8_t  byte[4];
} longUnion;
 ...
 * 
 ****************************************************************/
void TinyGPS_HJOE::printHex(uint8_t *data, uint8_t length) // prints 8-bit data in hex
{
  char tmp[length*2+1];
  byte first ;
  int j=0;
...
void TinyGPS_HJOE::ubx_checksum(byte ubx_data) {
  ck_a+=ubx_data;
  ck_b+=ck_a; 
}

void TinyGPS_HJOE::calcChecksum(byte *checksumPayload, byte payloadSize) {
  byte CK_A = 0, CK_B = 0;
